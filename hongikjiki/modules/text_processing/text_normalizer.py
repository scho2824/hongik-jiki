...
class TextNormalizer:
    """
    텍스트 정규화를 담당하는 클래스
    타임스탬프 제거, 특수문자 처리, 텍스트 정리 등을 수행하며,
    타임스탬프 제거뿐만 아니라 의미 단위 기반의 지능적인 문단 청크 분할에도 사용됩니다.
    """
    ...
    def normalize(self, text: str) -> str:
        ...
        # 2. 연속된 공백을 단일 공백으로 치환하여 정리
        text = re.sub(r'\s+', ' ', text)
        
        # 3. 연속된 개행 문자를 2개로 제한하여 문단 구분 유지
        text = re.sub(r'\n\s*\n\s*\n+', '\n\n', text)
    ...
    def chunk_text(self, text: str, max_length: int = 500) -> list[str]:
        ...
        # 문단 단위로 먼저 분할하고 공백 제거
        paragraphs = [p.strip() for p in text.split('\n\n') if p.strip()]
        chunks = []
        for para in paragraphs:
            if len(para) <= max_length:
                # 문단 길이가 제한 이내면 그대로 추가
                chunks.append(para)
            else:
                ...
                for sentence in sentences:
                    # 현재 청크에 문장 추가 후 길이 검사
                    if len(chunk) + len(sentence) <= max_length:
                        chunk += sentence + " "
                    else:
                        # 길이 초과 시 현재 청크 저장 후 새 청크 시작
