...
class TagAwareSearch:
    ...
    def rerank_results_by_tags(self, results: List[Dict[str, Any]], 
        ...
        for result in results:
            ...
            if not isinstance(doc_id, str):
                # Skip results without a valid document ID
                continue
    ...
    def extract_tags_from_query(self, query: str, tag_schema: Optional[Any] = None) -> Tuple[str, List[str]]:
        """
        Extract tag references from a user query using TagSchema if available

        Args:
            query: User query
            tag_schema: Optional TagSchema object for valid tag names

        Returns:
            Tuple of (clean_query, extracted_tags)
        """
        if tag_schema:
            all_tags = tag_schema.get_all_tags()
        ...
        # Additionally: simple word matching if no explicit marks
        for tag in tag_names:
            if tag in clean_query and tag not in extracted_tags:
                extracted_tags.append(tag)
