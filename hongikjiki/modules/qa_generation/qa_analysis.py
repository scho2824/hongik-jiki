...
def main():
    # 파일 경로 설정
    qa_file = ROOT_DIR / "data" / "qa" / "jungbub_qa_dataset.json"
    output_csv = ROOT_DIR / "data" / "qa" / "qa_tag_evaluation.csv"
    schema_file = ROOT_DIR / "data" / "converted_tag_schema.yaml"
    # 태그 스키마 및 추출기 로드
    tag_schema = TagSchema.load_from_yaml(str(schema_file))
    tag_extractor = TagExtractor(tag_schema)

    # QA 데이터 불러오기
    with open(str(qa_file), "r", encoding="utf-8") as f:
        qa_data = json.load(f)
    ...
    with open(str(output_csv), "w", encoding="utf-8", newline="") as f:
