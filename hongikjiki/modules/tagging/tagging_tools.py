...
from hongikjiki.modules.tagging.tag_schema import TagSchema, Tag
...
class TaggingSession:
    ...
    def load_document(self, document_id: str, content: str, 
        ...
        # Extract suggested tags - 수정된 부분: 반환 형식에 맞게 처리
        tags_result = self.tag_extractor.extract_tags(content, existing_tags)
        # 반환 형식 확인 및 변환
        if isinstance(tags_result, tuple) and len(tags_result) == 2:
            main_tags, near_tags = tags_result
            # main_tags가 비어있으면 near_tags에서 상위 태그 사용
            self.suggested_tags = main_tags if main_tags else {tag: score for tag, score in near_tags[:2]}
        else:
            # 이전 버전 호환성을 위한 처리
            self.suggested_tags = tags_result
        
        # Initialize manual tags with existing tags
        if existing_tags:
            tags_field = metadata.get("tags") if metadata is not None else None
            if isinstance(tags_field, dict):
                # If tags are already a dict with scores, use as-is
                self.manual_tags = tags_field
            else:
    ...
    def accept_suggested_tag(self, tag_name: str) -> bool:
        ...
        if tag_name in self.suggested_tags:
            if isinstance(self.manual_tags, dict):
...
class TaggingBatch:
    ...
    def process_document(self, document_id: str, content: str, 
        ...
        # Extract tags - 수정된 부분: 반환 형식에 맞게 처리
        tags_result = self.tag_extractor.extract_tags(content, existing_tags)
        # 반환 형식 확인 및 변환
        if isinstance(tags_result, tuple) and len(tags_result) == 2:
            main_tags, near_tags = tags_result
            # main_tags가 비어있으면 near_tags에서 상위 태그 사용
            tags = main_tags if main_tags else {tag: score for tag, score in near_tags[:2]}
        else:
            # 이전 버전 호환성을 위한 처리
            tags = tags_result
        ...
        for filename in os.listdir(self.output_dir):
            if filename.endswith("_tags.json"):
                try:
                    ...
                    tags_field = data.get("tags") if data is not None else None
                    tags_data = tags_field if tags_field is not None else {}
...
class TagValidationTool:
    ...
    def validate_document_tags(self, document_id: str, content: str, 
        ...
        keys = tag_dict.keys() if isinstance(tag_dict, dict) else []
        for tag_name in list(keys):
            if tag_name not in self.tag_schema.tags:
                validation_results["suspicious_tags"].append({
                    "tag": tag_name,
                    "reason": "Tag not found in schema",
                    "confidence": tag_dict[tag_name] if isinstance(tag_dict, dict) else 1.0
                })
            else:
    ...
    def batch_validate_tags(self, tag_dir: str, output_file: str) -> Dict[str, Any]:
        ...
        for filename in os.listdir(tag_dir):
            ...
            try:
                ...
                # 안전하게 document_id 값 가져오기
                document_id = data.get("document_id", "") if data is not None else ""
                # None인 경우 빈 문자열로 처리
                ...
                if "content" not in data:
                    ...
                    keys = tags.keys() if isinstance(tags, dict) else []
                    for tag_name in keys:
                        if tag_name in self.tag_schema.tags:
                            ...
                            validation["suspicious_tags"].append({
                                "tag": tag_name,
                                "reason": "Tag not found in schema",
                                "confidence": tags.get(tag_name, 1.0) if isinstance(tags, dict) else 1.0
                            })
